...
ACTIVE_EXCHANGES = [
    "US",          # Live S&P 500 + NASDAQ 100 from Wikipedia (~550 stocks)
    "BME",
    "XETRA",
    "EURONEXT",
    "AMS",
        ...
        for tbl in ndx_tables:
            for col in ['Ticker', 'Symbol']:
                if col in tbl.columns:
                    ndx_tickers = [str(x).replace('.', '-') for x in tbl[col].tolist() if isinstance(x, str) and x.strip()]
                    break
...
# ── European tickers (Yahoo Finance suffixes) ─────────────────────
EUROPEAN_TICKERS = {
...
def build_email(results):
    ...
    def row(r):
        ...
        return f"""<tr style="background:{bg}">
          <td style="padding:10px 14px;border-bottom:1px solid #1e1e27"><b style="color:#e8e8f0;font-size:14px">{r['flag']} {r['symbol']}</b><br><span style="color:#666;font-size:11px">{r['name']} · {r['exchange']}</span></td>
    ...
    def section(title, color, rows):
        if not rows: return ''
        return f"""<h2 style="color:{color};font-size:15px;margin:28px 0 8px;font-family:sans-serif">{title} — {len(rows)} stocks</h2>
        <table width="100%" style="border-collapse:collapse;background:#111118;border-radius:8px;overflow:hidden">
        <thead>{thead}</thead><tbody>{''.join(row(r) for r in rows)}</tbody></table>"""
    conds, c = [], CONDITIONS
    ...
    return f"""<!DOCTYPE html><html><body style="background:#0a0a0f;color:#e8e8f0;font-family:sans-serif;padding:28px;max-width:900px;margin:auto">
      ...
      <div style="display:flex;gap:16px;margin-bottom:28px;flex-wrap:wrap">
        <div style="background:#111118;border:1px solid #1e1e27;border-radius:8px;padding:14px 22px"><div style="font-size:26px;font-weight:700">{len(results)}</div><div style="font-size:11px;color:#555;margin-top:3px">Total matches</div></div>
        <div style="background:#111118;border:1px solid #22c55e44;border-radius:8px;padding:14px 22px"><div style="font-size:26px;font-weight:700;color:#22c55e">{len(strong)}</div><div style="font-size:11px;color:#555;margin-top:3px">Strong signals</div></div>
        <div style="background:#111118;border:1px solid #f59e0b44;border-radius:8px;padding:14px 22px"><div style="font-size:26px;font-weight:700;color:#f59e0b">{len(medium)}</div><div style="font-size:11px;color:#555;margin-top:3px">Medium signals</div></div>
        <div style="background:#111118;border:1px solid #1e1e27;border-radius:8px;padding:14px 22px"><div style="font-size:14px;font-weight:600;color:#888">{', '.join(ACTIVE_EXCHANGES)}</div><div style="font-size:11px;color:#555;margin-top:3px">Exchanges scanned</div></div>
      </div>
      {section('⭐ Strong signals', '#22c55e', strong)}
      {section('◑ Medium signals', '#f59e0b', medium)}
      <div style="margin-top:28px;padding-top:14px;border-top:1px solid #1e1e27;font-size:11px;color:#333">Data: Yahoo Finance · Indices: live from Wikipedia · RS = 52w range position</div>
    </body></html>"""
...
def send_email(results):
    ...
    msg['Subject'] = f"📈 Screener {now} — {len(results)} matches ({strong} strong)"
